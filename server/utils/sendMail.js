...
// Gmail transporter — works with Gmail App Password
...
const sendMail = async (to, subject, html) => {
  try {
    ...
    // Do not throw — email failure should not crash the server
  }
};
...
<div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;padding:20px;border:1px solid #eee;border-radius:8px;">
  ...
  <p style="color:#888;font-size:12px;margin-top:20px;">Developer: Anuj | anujsiwach002@gmail.com | PGDCA 2025-26</p>
</div>`;
...
<div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;padding:20px;border:1px solid #eee;border-radius:8px;">
  ...
  <p><strong>Total:</strong> Rs.${order.totalAmount.toLocaleString('en-IN')}</p>
  ...
  <div style="background:#F8F8F8;padding:15px;border-radius:6px;margin:15px 0;">
    ${order.items.map(i => `<div style="margin:5px 0;">Order: ${i.title} x ${i.qty} - Rs.${(i.price*i.qty).toLocaleString('en-IN')}</div>`).join('')}
  </div>
